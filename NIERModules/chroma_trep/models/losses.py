...
pred_loss_methods = {
    ...
    # Note We can consider Huber Loss and Smooth L1 Loss are same when hyperparameter delta=1
    # https://pytorch.org/docs/stable/generated/torch.nn.HuberLoss.html
    # "SMOOTH_L1": nn.SmoothL1Loss(), # Smooth L1 Loss
}
    ...
    while h1.size(1) > 1:
        ...
        if d >= temporal_unit:
            if weights['tembed_jsd_pred'] != 0:
                jsd_loss = tembed_jsd_pred_loss(h1, h2, tau1, tau2, tembed_jsd_task_head, pred_loss_method=pred_loss_method)
            ...
            if weights['tembed_cond_pred'] != 0:
                pred_loss = tembed_cond_pred_loss(h1, h2, tau1, tau2, tembed_pred_task_head, pred_loss_method=pred_loss_method) 
...
def temporal_contrastive_loss(h1, h2):
    ...
    # Remove Self-Similarity, (Remove diagonal Components)
...
def tembed_jsd_pred_loss(h1, h2, tau1, tau2, tembed_jsd_task_head, pred_loss_method='MSE'):
    """
    ...
    # loss = nn.MSELoss()
    loss = pred_loss_methods[pred_loss_method]
...
def tembed_cond_pred_loss(h1, h2, tau1, tau2, task_head, max_time_dist=10, pred_loss_method='MSE'):
    """
