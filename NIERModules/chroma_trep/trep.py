...
class TRep:
    ...
    def __init__(
        ...
        Args:
            ...
            after_epoch_callback (Callable): A callback function that would be called after each epoch.
            pred_loss (str): The loss function used for the forecasting task. Default: 'MSE'
            train_data_for_logging (np.ndarray): The training data used for logging the representation. This is used for visualization purposes and can be set to None.
        '''
        ...
        self.pred_loss_method = pred_loss_method
        # MODIFIED: Added the following line
        self.train_data_for_logging = train_data_for_logging
        self.train_labels_for_logging = train_labels_for_logging
        ...
        print(f"Task weights: {self.task_weights}, sum={sum(self.task_weights.values())}")
        assert round(sum(self.task_weights.values()), 6) == 1.0
        ...
        while True:
            ...
            for batch in train_loader:
                ...
                loss = hierarchical_contrastive_loss(
                    ...
                    pred_loss_method=self.pred_loss_method
                )
                
                loss.backward()
                ### MODIFIED: Added the following line
                # Gradient clipping
                if self.pred_loss_method == 'MSE':
            ...
            if hasattr(self, "_log_representation") and (self.n_epochs  % 5 == 0):
                self._log_representation(self.n_epochs) 
    ...
    def _log_representation(self, epoch):
        sample_size = 500

        if not hasattr(self, "train_data_for_logging"):
            return

        data = self.train_data_for_logging
        labels = self.train_labels_for_logging if hasattr(self, "train_labels_for_logging") else None
        ...
        reprs = self.encode(data, encoding_window='full_series')
